...
function AgentStatusRow({ nodeId, agentName, status, output, error, runId }) {
  ...
  const doApprove = async () => { setActioning(true); await api.approveNode(runId, nodeId); setActioning(false); };
      ...
      {isHITL && (
        <div className="run-row__hitl">
          ...
          <div className="run-row__hitl-actions">
            <button className="btn btn--success" onClick={doApprove} disabled={actioning}>✓ Approve</button>
            <button className="btn btn--danger" onClick={() => setRejecting((r) => !r)}>✗ Reject</button>
          </div>
          {rejecting && (
            <div className="run-row__reject-form">
              ...
              <button className="btn btn--danger" onClick={doReject} disabled={actioning}>Confirm Rejection</button>
            </div>
          )}
        </div>
      )}
...
export default function RunStatusOverlay() {
  ...
  const onMouseDown = (e) => {
    ...
    drag.current = { active: true, startX: e.clientX, startY: e.clientY, originX: rect.left, originY: rect.top };
  };

  useEffect(() => {
    const onMove = (e) => {
      if (!drag.current.active) return;
      setPos({ x: drag.current.originX + e.clientX - drag.current.startX,
  ...
  useEffect(() => { if (activeRunId) setPos(null); }, [activeRunId]);
  ...
  const getLabel = (el) => {
    const props = elementProps[el.id] ?? {};
    if (props.agentForgeType === "agent") return `🤖 ${el.label}`;
    if (props.agentForgeType === "mcp")   return `🔌 ${el.label} (${props.toolName ?? ""})`;
    if (el.bpmnType === "bpmn:IntermediateCatchEvent") return `⏱ ${el.label} (${props.timerValue ?? ""}s)`;
    if (el.bpmnType === "bpmn:ExclusiveGateway") return `◆ ${el.label}`;
    return el.label;
  };

  return (
    <div className="run-overlay" ref={overlayRef} style={posStyle}>
      <div className="run-overlay__header" onMouseDown={onMouseDown}
      ...
      <div className="run-overlay__rows">
        {executableElements.map((el) => (
          <AgentStatusRow
            key={el.id}
            nodeId={el.id}
            agentName={getLabel(el)}
            status={runStatus[el.id] ?? "not-started"}
            output={runOutputs[el.id]}
            error={null}
            runId={activeRunId}
          />
        ))}
